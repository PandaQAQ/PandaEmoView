...
public class StickerManager {
    ...
    public void addZipResource(String zipPath) {
        ZipUtils.unZipFiles(zipPath, EmoticonManager.getInstance().getStickerPath());
        EmoticonManager.getInstance().getManagedView().reloadEmos(0);
    }
    ...
    private void loadStickerData(StickerCategory category) {
        ...
        File stickerDir = new File(EmoticonManager.getInstance().getStickerPath(), category.getName());
        if (stickerDir.exists()) {
    ...
    private class EntryLoader extends DefaultHandler {
        ...
        List<StickerItem> load(StickerCategory category) {
            mCategory = category;
            stickerPath = new File(EmoticonManager.getInstance().getStickerPath(), category.getName()).getAbsolutePath();
            InputStream is = null;
            try {
